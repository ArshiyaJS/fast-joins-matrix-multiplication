...
def run_comprehensive_benchmark():
    ...
    def evaluate_and_record(exp_name, topology_name, config_params, tables, plan):
        ...
        for algo_name in algorithms:
            ...
            # Handle algorithm-specific parameter grids (smaller partition counts for quick testing)
            partition_configs = [16]
            if algo_name in ["grace_hash", "radix_hash"]:
                partition_configs = [4, 16]
    ...
    skew_values = [0.0, 1.2]  # Reduced set for quick test
    for z in skew_values:
        print(f"Running Experiment 1 [Skew Impact] with z={z}...")
        R, S = generator.generate_matrix_query(N=2_000, n_A=1_000, n_B=200, n_C=1_000, skew_b=z)
        tables = {'R': R, 'S': S}
        plan = [{'left': 'R', 'right': 'S', 'on': 'B', 'lsuffix': '_left', 'rsuffix': '_right'}]
        params = {"num_tuples_N": 2_000, "n_B": 200, "skew_z": z}
    ...
    n_values = [1_000, 3_000]  # Smaller test scales
    scalability_skews = [1.0]
    for N in n_values:
        for z in scalability_skews:
            print(f"Running Experiment 2 [Scalability] with N={N}, skew={z}...")
            R, S = generator.generate_matrix_query(N=N, n_A=N//2, n_B=200, n_C=N//2, skew_b=z)
            tables = {'R': R, 'S': S}
            plan = [{'left': 'R', 'right': 'S', 'on': 'B', 'lsuffix': '_left', 'rsuffix': '_right'}]
            params = {"num_tuples_N": N, "n_B": 200, "skew_z": z}
    ...
    selectivity_nb_values = [50, 500]
    for n_B in selectivity_nb_values:
        print(f"Running Experiment 3 [Selectivity] with n_B={n_B}...")
        R, S = generator.generate_matrix_query(N=2_000, n_A=1_000, n_B=n_B, n_C=1_000, skew_b=1.0)
        tables = {'R': R, 'S': S}
        plan = [{'left': 'R', 'right': 'S', 'on': 'B', 'lsuffix': '_left', 'rsuffix': '_right'}]
        params = {"num_tuples_N": 2_000, "n_B": n_B, "skew_z": 1.0}
    ...
    if hasattr(generator, "generate_star_query"):
        ...
        for z in star_skews:
            ...
            center, leaves = generator.generate_star_query(N=2_000, num_leaves=2, domain_center_b=200, skew_b=z)
        ...
        report_lines.append("\n--- EXPERIMENT 4c: Selectivity Impact (Q_star) ---")
        star_nb_values = [50, 500]
        for n_B in star_nb_values:
            print(f"Running Experiment 4c [Star Selectivity] with domain size={n_B}...")
            center, leaves = generator.generate_star_query(N=2_000, num_leaves=2, domain_center_b=n_B, skew_b=1.0)
            tables = {'center': center, 'leaf_0': leaves[0], 'leaf_1': leaves[1]}
            plan = [
                {'left': 'center', 'right': 'leaf_0', 'on': 'B', 'lsuffix': '_c', 'rsuffix': '_l0'},
                {'left': 'intermediate_step_0', 'right': 'leaf_1', 'on': 'B', 'lsuffix': '_step0', 'rsuffix': '_l1'}
    ...
    if hasattr(generator, "generate_acyclic_path_query"):
        ...
        for z in line_skews:
            ...
            line_domains = [1_000, 200, 1_000]
            path_relations = generator.generate_acyclic_path_query(
                N=2_000, path_length=2, domain_sizes=line_domains, skew_factors=[z, z]
            )
        ...
        report_lines.append("\n--- EXPERIMENT 5c: Selectivity Impact (Q_line) ---")
        for mid_domain in [50, 400]:
            print(f"Running Experiment 5c [Line Selectivity] with mid_domain={mid_domain}...")
            line_domains = [1_000, mid_domain, 1_000]
            path_relations = generator.generate_acyclic_path_query(
                N=2_000, path_length=2, domain_sizes=line_domains, skew_factors=[1.0, 1.0]
            )
            tables = {'R1': path_relations[0], 'R2': path_relations[1]}
            plan = [{'left': 'R1', 'right': 'R2', 'on': 'X_1', 'lsuffix': '_r1', 'rsuffix': '_r2'}]
            params = {"num_tuples_N": 2_000, "mid_domain_size": mid_domain, "skew_z": 1.0}
    ...
    print(f"\nQuick Test Benchmark completed successfully!")
